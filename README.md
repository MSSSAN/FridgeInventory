...
이 프로젝트는 사용자가 냉장고 내부 사진을 업로드하면, 멀티모달 AI가 사진 속 식음료를 인식하고 이를 구조화된 인벤토리로 정리해주는 iOS 기반 서비스입니다.

사용자는 냉장고 안을 직접 하나씩 입력하지 않아도 됩니다. 냉장고 사진을 찍어 업로드하면 AI가 사진 속 식재료, 음료, 보관 용기, 유제품, 채소, 과일 등을 분석하고, 각 항목을 카테고리별로 분류합니다. 이후 앱은 AI가 반환한 데이터를 바탕으로 식음료 목록, 예상 유통기한, 보관 상태, 보관 가이드, 개별 아이템 썸네일을 화면에 표시합니다.

이 단계의 핵심 목표는 다음과 같습니다.
...
flowchart TD
    ...
    A --> I[이미지 키와 사용자 요청 전송]
    I --> D
    D --> J[AI 처리 Lambda]

    J --> K[S3에서 이미지 다운로드]
    J --> L[DynamoDB에서 이전 대화/상태 조회]
    J --> M[AI 프롬프트 생성]
    M --> N[Multimodal AI API]

    N --> O[식음료 인벤토리 JSON 반환]
    O --> J
    J --> P[DynamoDB에 결과 저장]
    J --> A
...
처음에는 이미지를 base64로 변환하여 API 요청에 직접 포함하는 방식도 고려할 수 있지만, 이 방식은 요청 크기가 커지고 API Gateway나 Lambda의 제한에 걸릴 수 있습니다.
...
냉장고 사진 한 장을 AI가 분석하여 식음료 목록, 유통기한, 보관 정보, 아이템 썸네일을 자동 생성하는 스마트 냉장고 인벤토리 서비스입니다.
