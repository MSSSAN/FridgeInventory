...
사용자가 냉장고 내부 사진을 업로드하면, 멀티모달 AI가 사진 속 식음료를 인식하고 이를 구조화된 인벤토리로 정리해주는 iOS 어플리케이션.

냉장고 사진을 찍어 업로드하면 AI가 사진 속 식음료 제품들을 분석하고, 각 항목을 카테고리별로 분류함. 이후 앱은 AI가 반환한 데이터를 바탕으로 식음료 목록, 예상 유통기한, 보관 상태, 보관 가이드, 개별 아이템 썸네일을 화면에 표시함.
...
기존 방식에서는 사용자가 냉장고 안의 물건을 직접 하나씩 입력해야 했지만, 이 서비스에서는 사진 한 장 또는 여러 장을 업로드하는 것만으로 냉장고 안의 식음료 정보를 자동 추출하도록 설계함.

AI는 업로드된 이미지를 분석하여 다음과 같은 정보를 생성함.
